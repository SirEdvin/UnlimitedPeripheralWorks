...
import site.siredvin.peripheralworks.util.StorageDealer
import java.util.Optional
import java.util.function.Predicate

class GenericFluidTransferPlugin(private val storage: Storage<FluidVariant>): IPeripheralPlugin {

    companion object {
        val FORGE_COMPACT_DEVIDER = 81.0
    }
    ...
    fun pushFluid(computer: IComputerAccess, toName: String, limit: Optional<Long>, fluidName: Optional<String>): Double {
        ...
        val toStorage = StorageDealer.extractFluidStorage(location.target)
        ...
        return StorageUtil.move(storage, toStorage, predicate, limit.orElse(Long.MAX_VALUE), null) / FORGE_COMPACT_DEVIDER
    }
    ...
    fun pullFluid(computer: IComputerAccess, fromName: String, limit: Optional<Long>, fluidName: Optional<String>): Double {
        ...
        val fromStorage = StorageDealer.extractFluidStorage(location.target)
        ...
        return StorageUtil.move(fromStorage, storage, predicate, limit.orElse(Long.MAX_VALUE), null) / FORGE_COMPACT_DEVIDER
    }
}