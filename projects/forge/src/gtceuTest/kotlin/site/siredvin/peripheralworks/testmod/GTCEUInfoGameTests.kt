...
class GTCEUInfoGameTests {
    ...
    fun machineSnapshot(helper: GameTestHelper) {
        ...
        val energy = entity.getCapability(GTCapability.CAPABILITY_ENERGY_CONTAINER).resolve().orElseThrow()
        val logic = entity.getCapability(GTCapability.CAPABILITY_RECIPE_LOGIC).resolve().orElseThrow()
        ...
        val before = entity.saveWithoutMetadata()
        check(plugin.getInfo() == idle)
        check(entity.saveWithoutMetadata() == before) { "Snapshot mutated machine data" }

        for (eu in listOf(32L, -32L)) {
            val recipe = GTRecipeBuilder(ResourceLocation("peripheralworks_testmod", "info"), machine.definition.recipeTypes.first())
                .duration(100).EUt(eu).buildRawRecipe()
    ...
    fun maintenanceAndControllerLifecycle(helper: GameTestHelper) {
        ...
        val energy = energyHatch.holder.self().getCapability(GTCapability.CAPABILITY_ENERGY_CONTAINER).resolve().orElseThrow()
        ...
        try {
            ...
            check(section(formed, "multiblock")["batchEnabled"] == true)
    ...
    fun luaSnapshot(helper: GameTestHelper) {
        ...
        helper.level.setBlockAndUpdate(pos, BuiltInRegistries.BLOCK.get(ResourceLocation("gtceu", "lv_macerator")).defaultBlockState())
        val machine = (helper.level.getBlockEntity(pos) as IMachineBlockEntity).metaMachine
        machinePlugin(machine)
        machine.holder.self().getCapability(GTCapability.CAPABILITY_ENERGY_CONTAINER).resolve().orElseThrow().addEnergy(1234)
