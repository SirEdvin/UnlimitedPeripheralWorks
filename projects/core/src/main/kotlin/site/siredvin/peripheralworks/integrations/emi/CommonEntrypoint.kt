...
object CommonEntrypoint {
    ...
    fun mapStack(stack: EmiStack): MutableMap<String, Any> {
        ...
        base["chance"] = stack.chance
        val components = DataComponentPatch.CODEC.encodeStart(NbtOps.INSTANCE, stack.componentChanges).result().getOrNull()
        if (components != null) {
            val nbtHash = ComputerPlatformToolkit.get().nbtHash(components)
            if (nbtHash != null) {
                base["nbt"] = nbtHash
            }
        }
