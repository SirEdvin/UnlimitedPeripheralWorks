...
class UltimateConfigurator : DescriptiveItem(Properties().stacksTo(1)) {
    ...
    fun getActiveMode(stack: ItemStack): Pair<ConfigurationMode, BlockPos>? {
        ...
        val pos = NbtUtils.readBlockPos(data, ACTIVE_MOD_POS).orElse(null) ?: return null
        return Pair(
            configurationMode,
            pos,
        )
    ...
    private fun readTargets(stack: ItemStack, key: String, limit: Int): List<ConfiguratorTarget> {
        val data = stack.get(DataComponents.CUSTOM_DATA)?.copyTag() ?: return emptyList()
    ...
    private fun writeTargets(stack: ItemStack, key: String, targets: List<ConfiguratorTarget>, limit: Int) {
        ...
        CustomData.update(DataComponents.CUSTOM_DATA, stack) { it.put(key, list) }
    }
    ...
    fun saveActiveMode(stack: ItemStack, mode: ConfigurationMode, targetBlock: BlockPos, level: Level) {
        ...
        CustomData.update(DataComponents.CUSTOM_DATA, stack) {
            it.putString(ACTIVE_MOD_NAME, mode.modeID.toString())
            it.put(ACTIVE_MOD_POS, NbtUtils.writeBlockPos(targetBlock))
            it.putString(ACTIVE_MOD_DIMENSION, level.dimension().location().toString())
        }
    ...
    fun clearActiveMode(stack: ItemStack): ItemStack {
        getActiveMode(stack)?.first?.clearData(stack)
        CustomData.update(DataComponents.CUSTOM_DATA, stack) {
            it.remove(ACTIVE_MOD_NAME)
            it.remove(ACTIVE_MOD_POS)
            it.remove(ACTIVE_MOD_DIMENSION)
        }
        return stack
    }
    ...
    fun getFavoriteTextStyle(stack: ItemStack): TextStyle = stack.get(DataComponents.CUSTOM_DATA)?.copyTag()?.getString(FAVORITE_TEXT_STYLE)?.let { runCatching { TextStyle.valueOf(it) }.getOrNull() } ?: TextStyle.REGULAR

    fun getFavoriteBoxStyle(stack: ItemStack): BoxStyle = stack.get(DataComponents.CUSTOM_DATA)?.copyTag()?.getString(FAVORITE_BOX_STYLE)?.let { runCatching { BoxStyle.valueOf(it) }.getOrNull() } ?: BoxStyle.OUTLINE

    fun setSettings(stack: ItemStack, name: String?, textStyle: TextStyle? = null, boxStyle: BoxStyle? = null): Boolean {
        if (name != null) {
            if (name.length > ConfiguratorTarget.MAX_NAME_LENGTH) return false
            if (name.isEmpty()) stack.remove(DataComponents.CUSTOM_NAME) else stack.set(DataComponents.CUSTOM_NAME, Component.literal(name))
        }
        CustomData.update(DataComponents.CUSTOM_DATA, stack) {
            textStyle?.let { style -> it.putString(FAVORITE_TEXT_STYLE, style.name) }
            boxStyle?.let { style -> it.putString(FAVORITE_BOX_STYLE, style.name) }
        }
        return true
    }
