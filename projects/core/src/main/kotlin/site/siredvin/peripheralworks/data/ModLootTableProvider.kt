...
object ModLootTableProvider {
    private val ae2BlockHooks = mutableListOf<BiConsumer<LootTableHelper, BiConsumer<ResourceLocation, LootTable.Builder>>>()

    fun addAE2BlockHook(hook: BiConsumer<LootTableHelper, BiConsumer<ResourceLocation, LootTable.Builder>>) {
    ...
    fun registerBlocks(output: BiConsumer<ResourceLocation, LootTable.Builder>, integration: Boolean = false) {
        val consumer = BiConsumer<ResourceLocation, LootTable.Builder> { id, table ->
        ...
        val integrationConsumer = BiConsumer<ResourceLocation, LootTable.Builder> { id, table ->
            if (integration) output.accept(id, table)
        }
