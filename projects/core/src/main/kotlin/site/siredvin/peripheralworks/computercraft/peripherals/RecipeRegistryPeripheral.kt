...
class RecipeRegistryPeripheral(
    ...
    fun get(recipeID: String): MethodResult {
        ...
        return MethodResult.of(RecipeRegistryToolkit.serializeRecipe(recipe, peripheralOwner.level!!.registryAccess()))
    }
    ...
    fun getRaw(recipeID: String): MethodResult {
        ...
        @Suppress("DEPRECATION", "KotlinRedundantDiagnosticSuppress")
        val recipePath = ResourceLocation(
            recipeId.namespace,
            "recipes/" + recipeId.path + ".json",
        )
        val resourceManager = PlatformToolkit.get().minecraftServer?.resourceManager ?: return MethodResult.of(null, "Cannot find server variabel")
        try {
    ...
    fun list(arguments: IArguments): MethodResult {
        ...
        return MethodResult.of(peripheralOwner.level!!.recipeManager.recipes.filter { recipeTypes.contains(it.type) }.map { it.id.toString() })
    }
