...
internal abstract class FilterDeviceObject<T : Any>(
    ...
    fun setFilter(slot: Int, resource: Map<*, *>) {
        ...
        if (!inventory.isAllowedIn(slot - 1, stack.what())) throw LuaException("Resource is not supported by this device")
...
internal open class InterfaceObject(level: Level, resolve: () -> InterfaceLogicHost) :
    ...
    fun setStock(slot: Int, target: Map<*, *>) {
        ...
        if (!config.isAllowedIn(slot - 1, stack.what())) throw LuaException("Resource is not supported by this interface")
...
internal class StorageLevelEmitterObject(
    ...
    fun setMonitoredResource(resource: Map<*, *>) {
        val target = device()
        val stack = AE2Helper.parseResource(resource, false)
        if (!target.config.isAllowedIn(0, stack.what())) throw LuaException("Resource is not supported by this emitter")
