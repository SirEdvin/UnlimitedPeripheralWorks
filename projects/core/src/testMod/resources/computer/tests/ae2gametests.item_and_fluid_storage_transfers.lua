local function wait_for_storage(side)
    ...
    error("ME network peripheral did not become available on " .. side .. ": " .. textutils.serialize(peripheral.getMethods(side)))
end
...
assert(item_count(primary) == 8, "item listing did not expose the seeded diamonds: " .. textutils.serialize({front.items(), back.items()}))
