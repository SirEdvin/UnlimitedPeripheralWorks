    ...
    fun createsHierarchyAndAddsPeripherals(helper: GameTestHelper) {
        ...
        helper.startSequence()
            .thenExecute {
                ...
                data.putString(UltimateConfigurator.ACTIVE_MOD_NAME, NetworkManagerMode.modeID.toString())
                data.put(UltimateConfigurator.ACTIVE_MOD_POS, NbtUtils.writeBlockPos(helper.absolutePos(managerPos)))
                data.putString(UltimateConfigurator.ACTIVE_MOD_DIMENSION, helper.level.dimension().location().toString())
