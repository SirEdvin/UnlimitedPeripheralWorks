...
class AE2PatternPedestalGameTests {
    ...
    fun invalidPatternsPersistenceAndInteraction(helper: GameTestHelper) {
        ...
        val used = entity.blockState.useItemOn(player.mainHandItem, helper.level, player, InteractionHand.MAIN_HAND, BlockHitResult(Vec3.atCenterOf(absolute), Direction.UP, absolute, false))
        check(used == ItemInteractionResult.CONSUME && player.mainHandItem.count == 63 && entity.storedStack.count == 1)
        player.setItemInHand(InteractionHand.MAIN_HAND, ItemStack.EMPTY)
        @Suppress("DEPRECATION")
        entity.blockState.attack(helper.level, absolute, player)
        ...
        val processing = PatternDetailsHelper.encodeProcessingPattern(listOf(GenericStack(AEItemKey.of(Items.STONE), 1)), listOf(GenericStack(AEItemKey.of(Items.DIAMOND), 1)))
        check(entity.replacePattern(entity.storedStack.copy(), processing))
        val saved = entity.saveWithoutMetadata(helper.level.registryAccess())
        val loaded = AE2PatternPedestalBlockEntity(absolute, entity.blockState)
        loaded.loadWithComponents(saved, helper.level.registryAccess())
        ...
        synchronized.loadWithComponents(entity.getUpdateTag(helper.level.registryAccess()), helper.level.registryAccess())
    ...
    fun nativeRecipesAndLimits(helper: GameTestHelper) {
        ...
        fun item(stack: ItemStack) = site.siredvin.peripheralworks.integrations.ae2.AE2Helper.keyToMap(AEItemKey.of(stack)!!) + mapOf("count" to 1, "snbt" to DataComponentPatch.CODEC.encodeStart(ops, stack.componentsPatch).getOrThrow().toString())
        ...
        entity.storedStack.set(DataComponents.CUSTOM_DATA, CustomData.of(CompoundTag().apply { putString("preserve", "blank tag") }))
        ...
        val cake = PatternDetailsHelper.decodePattern(entity.storedStack.copy(), helper.level) as appeng.crafting.pattern.AECraftingPattern
        val grid = CraftingInput.of(3, 3, cakeItems.map(::ItemStack))
        check(cake.getRemainingItems(grid).count { it.`is`(Items.BUCKET) } == 3)
        val missing = entity.storedStack.copy().apply {
            val encoded = get(AEComponents.ENCODED_CRAFTING_PATTERN)!!
            set(AEComponents.ENCODED_CRAFTING_PATTERN, EncodedCraftingPattern(encoded.inputs(), encoded.result(), ResourceLocation.parse("minecraft:missing_recipe"), encoded.canSubstitute(), encoded.canSubstituteFluids()))
        }
        ...
        val cutting = helper.level.recipeManager.getAllRecipesFor(net.minecraft.world.item.crafting.RecipeType.STONECUTTING).filter { it.value().matches(SingleRecipeInput(input), helper.level) }.take(2)
        ...
        for (recipe in cutting) {
            ...
            val native = PatternDetailsHelper.decodePattern(entity.storedStack.copy(), helper.level) as appeng.crafting.pattern.AEStonecuttingPattern
            check(native.recipeId == recipe.id && native.canSubstitute)
            check((native.outputs.single().what as AEItemKey) == AEItemKey.of(recipe.value().assemble(SingleRecipeInput(input), helper.level.registryAccess())))
            check(peripheral.clearPattern().result!![0] == true)
        }
        val template = ItemStack(Items.SENTRY_ARMOR_TRIM_SMITHING_TEMPLATE)
        val base = ItemStack(Items.IRON_CHESTPLATE).apply { damageValue = 7 }
        val addition = ItemStack(Items.DIAMOND)
        val smithingInputs = SmithingRecipeInput(template, base, addition)
        val trim = helper.level.recipeManager.getAllRecipesFor(net.minecraft.world.item.crafting.RecipeType.SMITHING).first { it.value().matches(smithingInputs, helper.level) }
        ...
        val encodedTrim = PatternDetailsHelper.decodePattern(entity.storedStack.copy(), helper.level) as appeng.crafting.pattern.AESmithingTablePattern
        check(encodedTrim.canSubstitute && encodedTrim.recipeId == trim.id)
        check(encodedTrim.outputs.single().what == AEItemKey.of(trim.value().assemble(smithingInputs, helper.level.registryAccess())))
        check(peripheral.clearPattern().result!![0] == true)
