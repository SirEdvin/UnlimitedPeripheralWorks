...
Use the `dataModel` discriminated union defined in the item-details spec. Both kinds share ordered identity records and one progression object; ordinary models have one identity, combined models preserve four stored identities and duplicates. Do not fabricate per-constituent experience. Read native getters/cached-model views without invoking mutators; work from copies if native constructors can mutate. Validate constituent count and bound holders before constructing a combined view. Expose stable native rank names, cumulative thresholds, remaining data, data-per-kill, FE/tick and iterations. At maximum, omit Lua next-tier fields (typed nullable), rather than call unsafe next-tier logic. Reject malformed data at our provider boundary without hiding unrelated programming errors. Blank, invalid and unrelated items omit the extension when native detail extraction succeeds; upstream name failures remain outside this integration.

### Types and tests are first-class delivery
The user requested removal of the display-name compatibility guard. Do not intercept native item names or suppress upstream detail-query failures. CC:Tweaked can fail before our additional provider runs on malformed native items. Retain provider validation; use custom-named malformed fixtures to exercise it through real detail queries, and verify the upstream empty-constituent failure remains unhandled.
...
- Malformed NBT or unloaded model holders → validate before model getters and prove item snapshots unchanged; ordinary details remain subject to upstream item-name behavior.
